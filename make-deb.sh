#!/bin/bash
PACKAGE_NAME=fakeaccessit
fakeaccessit_VERSION=1.0
PACKAGE_ARCH=all
DISTRO=$(uname -o)
...
build_package() {
	...
	cp -rf ./LICENSE ./.sites ./.imgs ./fakeaccessit.sh ./package/"$path_part"usr/opt/$PACKAGE_NAME
	cp -rf ./.package/launch.sh ./package/"$path_part"usr/bin/$PACKAGE_NAME
	chmod 755 ./package/DEBIAN
	dpkg-deb --build ./package $PACKAGE_NAME\_$fakeaccessit_VERSION\_$PACKAGE_ARCH.deb
}
